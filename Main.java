...
class commands {
	...
	// Dosya şifresini kırma metodu:
	public static void fileDecryption(String oldFolderPath, String folderExtension) throws IOException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException {
	    ...
	    for (File file : files) {
	        if (folderExtension.equals("All")) {
	            if (file.isFile() && file.getName().endsWith(".encrypted")) {
	                ...
	                String outputFilePath = oldFolderPath + File.separator + file.getName().replace(".encrypted", "");
	                FileOutputStream outputStream = new FileOutputStream(outputFilePath);
...
public class Main extends JFrame {
	...
	private JCheckBox moveAndEncryptCheckBox, moveAndDecryptCheckBox, moveAndZipCheckBox, moveAndHideCheckBox, moveCheckBox;
	...
	public Main() {
		...
		moveAndDecryptCheckBox = new JCheckBox("Tasi ve Sifreyi Coz");
		...
		checkBoxGroup.add(moveAndDecryptCheckBox);
		...
		moveAndDecryptCheckBox.setBounds(100, 480, 150, 20);
		moveAndZipCheckBox.setBounds(100, 510, 150, 20);
		moveAndHideCheckBox.setBounds(100, 540, 150, 20);
		moveCheckBox.setBounds(100, 570, 150, 20);
		...
		frame.add(moveAndDecryptCheckBox);
	...
	private void runButtonClicked() throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IOException {
		...
		if (moveAndEncryptCheckBox.isSelected() || moveAndDecryptCheckBox.isSelected() || moveAndZipCheckBox.isSelected() || moveAndHideCheckBox.isSelected() || moveCheckBox.isSelected()) {
		} else {
		...
		if (moveAndEncryptCheckBox.isSelected()) {
			...
			if (allFilesRadioButton.isSelected()) {
				...
				JOptionPane.showMessageDialog(null, "Tüm dosyalar taşındı ve şifrelenebilecek formattaki dosyalar şifrelendi.\nİşlem tamamlandı.", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}
		...
		// Taşı ve Şifreyi Kır metodu seçilirse:
		if (moveAndDecryptCheckBox.isSelected()) {

			// Tüm dosyalara işlem yapılmak isteniyorsa:
			if (allFilesRadioButton.isSelected()) {
				commands.fileDecryption(oldAddressText, ".encrypted");
				commands.fileMoving(oldAddressText, newAddressText, "All");
				JOptionPane.showMessageDialog(null, "Tüm dosyalar taşındı ve şifrelenebilecek formattaki dosyaların şifresi çözüldü.\nİşlem tamamlandı.", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}

			// Yalnızca '.txt' uzantılı dosyalara işlem yapılmak isteniyorsa:
			else if (onlyTxtRadioButton.isSelected()) {
				commands.fileDecryption(oldAddressText, ".txt.encrypted");
				commands.fileMoving(oldAddressText, newAddressText, ".txt");
				JOptionPane.showMessageDialog(null, "Tüm '.txt' uzantılı dosyalar taşındı ve şifreleri çözüldü.\nİşlem tamamlandı.", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}

			// Yalnızca '.pdf' uzantılı dosyalara işlem yapılmak isteniyorsa:
			else if (onlyPdfRadioButton.isSelected()) {
				commands.fileDecryption(oldAddressText, ".pdf.encrypted");
				commands.fileMoving(oldAddressText, newAddressText, ".pdf");
				JOptionPane.showMessageDialog(null, "Tüm '.pdf' uzantılı dosyalar taşındı ve şifreleri çözüldü.\nİşlem tamamlandı.", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}

			// Yalnızca '.docx' uzantılı dosyalara işlem yapılmak isteniyorsa:
			else if (onlyDocxRadioButton.isSelected()) {
				commands.fileDecryption(oldAddressText, ".docx.encrypted");
				commands.fileMoving(oldAddressText, newAddressText, ".docx");
				JOptionPane.showMessageDialog(null, "Tüm '.docx' uzantılı dosyalar taşındı ve şifreleri çözüldü.\nİşlem tamamlandı.", "Bilgi", JOptionPane.INFORMATION_MESSAGE);
			}
		}
